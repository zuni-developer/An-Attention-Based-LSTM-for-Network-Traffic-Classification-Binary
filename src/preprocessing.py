...
# ---------------------------------------------------------------------------
# CIC-Darknet2020 hierarchical binary mapping
# ---------------------------------------------------------------------------
# Our approach to traffic categorization employs a hierarchical structure,
# allowing for multi-level classification of network flows. At the highest
# level, we distinguish between darknet and benign traffic in a binary
# classification. VPN and Tor traffic are labeled as Darknet, while
# Non-VPN and Non-Tor traffic are labeled as Benign.
#
# This mapping is applied automatically whenever the detected target
# column's classes match CIC-Darknet2020's fine-grained scheme (Tor /
# Non-Tor / VPN / Non-VPN, however spelled/spaced), so every other dataset
# (e.g. UNSW-NB15) is left completely untouched.
_DARKNET_LABEL_MAP = {
    "tor": "Darknet",
    "vpn": "Darknet",
    "nontor": "Benign",
...
def _normalize_label_token(value: str) -> str:
    """
    Lowercase and strip whitespace/hyphens/underscores for robust label
    matching, so 'Non-Tor', 'Non_Tor', 'NonTor', and 'nontor' all normalize
    to the same token (and likewise for 'NonVPN' / 'Non-VPN' / 'non_vpn').
    """
    return (
...
def is_cicdarknet_label_scheme(series: pd.Series) -> bool:
    """
    Detect whether a target series follows the CIC-Darknet2020 fine-grained
    label scheme (Tor / Non-Tor / VPN / Non-VPN), regardless of exact
    spelling/casing/separators used.
    """
    unique_tokens = {_normalize_label_token(v) for v in series.dropna().unique()}
    known_tokens = set(_DARKNET_LABEL_MAP.keys())
    return len(unique_tokens) > 0 and unique_tokens.issubset(known_tokens)


def apply_darknet_binary_mapping(series: pd.Series) -> pd.Series:
    """
    Map CIC-Darknet2020's four fine-grained classes to a hierarchical binary
    scheme: {Tor, VPN} -> 'Darknet', {Non-Tor, Non-VPN} -> 'Benign'.
    """
    mapped = series.astype(str).map(
        lambda v: _DARKNET_LABEL_MAP.get(_normalize_label_token(v), v)
    )
    logger.info(
        "Detected CIC-Darknet2020 label scheme -> applying hierarchical "
        "binary mapping: VPN & Tor => 'Darknet', Non-VPN & Non-Tor => 'Benign'."
    )
    return mapped
...
def preprocess(
    ...
    # If this is the CIC-Darknet2020 fine-grained label scheme, collapse it
    # to the hierarchical top-level binary classification: Darknet vs Benign.
    if is_cicdarknet_label_scheme(y_raw):
        y_raw = apply_darknet_binary_mapping(y_raw)
